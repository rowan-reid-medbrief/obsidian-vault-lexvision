...
- **Status:** OCR per-page timeout (`2804882`) and content-addressed matcher render cache (`0505505`) both SHIPPED + pushed 2026-07-04 (311 tests). Curated clean confirmation: the OCR timeout works in-situ (hit the pathological page, timed out, skipped, completed - the hang is retired) and tonight's changes are exonerated for split silent-errors by the render/OCR-independent stamped-arm control (same 2/10 silent SPLIT). Full-corpus zero-silent-SPLIT reconfirmation BLOCKED by a new `[high]`: at scale the SPLIT scenarios make many distinct thin-strip slivers, each paying a full 120s OCR timeout (>1hr run); fix = pre-emptive degenerate-page skip. `last touched` 2026-07-04.
- **next:** Fix the sliver OCR-timeout blowup (pre-emptive degenerate-page skip) -> then the now-fast full-corpus clean confirmation (zero silent SPLIT) + degraded pass -> then the sequence/neighbour layer (#5, RESOLVE near-identical/cross-section/exact-dup splits). On `feat/coverage-graph-reframe`.
- [[2026-07-04-mri133-ocr-timeout-and-render-cache]]
