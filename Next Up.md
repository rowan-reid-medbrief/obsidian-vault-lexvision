...
- **Status:** Real-data bake-off on Azure-sourced case docs surfaced and fixed 3 matcher harness bugs (discarded Hungarian assignment, boilerplate counted as content, a false-SPLIT storm) - real-data content-arm ensemble POOR->GOOD. Stamp-survival spike: a page-dict id is LOST through MedBrief's real `PagePushBack` merge, not just the flatten path, so the durable id should be the DB `Page.id` on the never-merged `Page.pdfFile`, not an in-PDF stamp. Built a pagination-sequence matcher (reads printed "Page X of Y"; closes the safety-critical REMOVED silence + resolves CLONE, but NOT split - the footer marker travels to one child only) and wired a `--population clean|degraded|both` axis (content arm collapses on scanned pages; only the stamp is population-independent). SPLIT is now confirmed unsolved by every channel. All merged to main. `last touched` 2026-07-02.
- **next:** New session reads the commissioned deep-research PDF (`docs/research/Cardinality-Aware...pdf`) FIRST, then re-ranks the split-detection plan; then `[high]` fuse the pagination channel into the content-arm ensemble. Handoff prompt drafted this session; real docs kept under `poc/data/real-*` for that work.
- [[2026-07-02-mri133-real-data-bakeoff-stamp-spike-pagination]]
