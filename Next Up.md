...
- **Status:** SPLIT is now zero-silent on the real corpus via two mechanisms added this session-arc: in-matcher OCR for image-only pages (retires 2 of 3) and an exact-duplicate twin SAFE-QUEUE (`twin_split_abstain`, default on, retires the 3rd). The twin case is information-theoretically unresolvable on page-local content - the split page and its surviving duplicate are byte-identical in text AND geometry (split residual 0.00000) - so it is safe-queued for review, not auto-placed; the rule is monotone-safe (only ever emits AMBIGUOUS, can never create a silent error). `feat/coverage-graph-reframe` @ 95f0e26 pushed; 300 tests. `last touched` 2026-07-03.
- **next:** Re-read the enlarged bake-off to confirm zero silent SPLIT at aggregate (it kept getting killed on OCR - run capped and/or fix the OCR perf/caching first); then the degraded pass (`--population degraded`), the dormant REMOVED rule keep/drop decision, the Deon OCR-at-ingestion recommendation, and the sequence/neighbour layer (to RESOLVE - not just safe-queue - near-identical / cross-section / exact-duplicate splits).
- [[2026-07-03-mri133-twin-split-safe-queue]]
