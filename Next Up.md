...
- **Status:** SPLIT detection built end to end from the commissioned deep-research report (splits are a *containment* problem, not *resemblance*): a token-space pipeline in `decision.py` (containment selection -> greedy weighted set-cover -> adjacency guard) took real clean SPLIT from 100% silent to 33% silent (50% recall / 60% precision) - confidently resolving a clean adjacent text split and safely queuing the rest - and a geometric substrate (`matchers/split_geometry.py`: RANSAC crop fit over distinctive word boxes) filters look-alikes by transform residual and supplies real source-region bands, holding the metric while generalising past the adjacency heuristic. Real-data bake-off harness reconstructed + committed (`scripts/real_bakeoff.py`). Two remaining SPLIT gaps - near-identical template pages and blank pages - proven to need sequence/neighbour context and the visual channel. 250 tests pass. `last touched` 2026-07-02.
- **next:** Decide merge of branch `feat/split-containment-detection` (2 commits) into main; then the `[high]` sequence / neighbour-consistency lever for near-identical splits, or `[high]` fuse the pagination channel into the content arm (still open). Real docs kept under `poc/data/real-*`.
- [[2026-07-02-mri133-split-detection-geometric]]
