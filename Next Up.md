...
- Translation | Compre (AI-2066) (spike; endpoint resolved + hands-on tested, last touched 2026-07-10)
  next: pick where to stage the PHI for Azure's batch PDF run. Sync refuses PDF; batch-only needs a blob source and target container. Options a/b/c/d in `docs/ai-2066/OVERVIEW.md`, then run `docs/ai-2066/harness/batch_translate.sh <account>`. Also still from Laura: which sample pages are German vs the existing English, the Greek attempt, benchmark tool accounts. Full ticket: `docs/ai-2066/`.
  [[2026-07-10-ai2066-azure-translation-pdf-batch-only]]
