# Meeting Graph Store

Curated, version-controlled copies of promoted meeting graphs. Each entry is a directory
holding the typed subset of a `/meeting-graph` run; resolution starts at its `graph.meta.json`.
Reverse-chronological. Auto-derived from the on-disk `graph.meta.json` files by `/promote-graph`;
manual edits here are overwritten on the next promotion.

- 2026-06-12 — Kumulus Regroup [lexvision/lexvision-poc] kumulus-regroup-2026-06-12
- 2026-06-08 — MedBrief Walkthrough Part 2 [medbrief/medbrief-onboarding] medbrief-walkthrough-part2-2026-06-08
- 2026-06-07 — Kumulus & Medbrief — Vendor Proposal [lexvision/lexvision-poc] kumulus-medbrief-2026-06-07
- 2026-06-07 — Data Engineering Discussion [lexvision/lexvision-poc] data-engineering-discussion-2026-06-07
- 2026-06-03 — MB Product Walk-through [medbrief/medbrief-onboarding] mb-product-walkthrough-2026-06-03
- ????-??-?? — Catch Up 2026-07-01 [medbrief/annotations] catch-up-2026-07-01
