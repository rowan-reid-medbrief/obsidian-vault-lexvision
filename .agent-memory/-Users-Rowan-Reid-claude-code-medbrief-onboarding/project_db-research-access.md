...
Gotcha (2026-06-19, MRI-133 re-review): the big MedBrief tables use `deletedAt` soft-deletes (`pdftron_annotation`, `document`, `sortingsession`, ...). Decide all-rows vs live (`deletedAt IS NULL`) up front and hold ONE filter across a finding: mixing them (an all-rows annotation count with live-only document/matter counts) silently misstates the population. The pre-staging instance is MySQL 5.7, schema `medbrief-live`.
