...
- [Cardinality channels](mri133-cardinality-channels.md): SPLIT zero-silent on the real corpus via 6 mechanisms (11-doc corpus surfaced 3 silent SPLITs the small corpus hid, retired by OCR + exact-dup twin safe-queue `twin_split_abstain` 2026-07-03; corrects the old "zero silent both populations"). Coverage-graph reframe DONE. Open: RESOLVING near-identical/cross-section/exact-dup splits needs the sequence layer; dormant REMOVED rule OFF; pagination `[parked]`.
