...
- [Cardinality channels](mri133-cardinality-channels.md): SPLIT now FULLY HANDLED (resolve-or-safe-queue, zero silent on the real corpus both populations) — token-space + geometric seam-selector cracks near-identical templates, degraded fixture bug fixed, blank-page gp flagged not silent. Pagination fuse `[parked]`; only gp confident-resolution stays open (needs cross-document evidence).
