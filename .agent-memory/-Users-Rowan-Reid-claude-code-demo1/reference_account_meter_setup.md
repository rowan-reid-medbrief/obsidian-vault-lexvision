...
**2026-07-16 clarification (Rowan):** despite the `personal` tag, this laptop (demo1) is where Lexvision/MedBrief work actually happens day to day — Lexvision is MedBrief's holding company. The `personal` tag governs billing only, not what work is done here; do not infer from MedBrief-flavoured MCP registrations (e.g. `claude mcp list` showing azure-devops/atlassian/HubSpot) that something is misconfigured.
